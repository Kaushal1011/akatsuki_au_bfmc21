...
app.layout = html.Div(
    [
        dcc.Store(id="stored_df", data=[], storage_type="memory"),
        dcc.Interval(id="html-div", interval=100, n_intervals=0),  # in milliseconds
        dcc.Interval(id="html-div1", interval=100, n_intervals=0),  # in milliseconds
        dbc.Container(
            [
                dbc.Row(
                    [
...
@app.callback(Output("stored_df", "data"), Input("html-div", "n_intervals"))
...
@app.callback(
    Output("position", "figure"), Input("interval-component-1", "n_intervals")
)
...
def update_speed_curve(data, n_intervals):
    ...
    dff = pd.DataFrame.from_records(data)
    fig_line = px.line(
        dff, x=dff.index, y="Speed", template="plotly_dark", title="Speed", markers=True
    )
...
@app.callback(Output("stored_df", "clear_data"), [Input("html-div-1", "n_intervals")])
