...
class LaneKeep:
    ...
    def houghlines_angle(self, img: np.ndarray) -> float:
        ...
        processed_img=cv2.Canny(processed_img,self.canny_thres1,self.canny_thres2)
    ...
    def graph_road_search(self,img:np.ndarray)->float:
        processed_img = self.preprocess_pipeline(img)
        angle,_=get_road_ratio_angle(processed_img)
...
def find_lanes(thresh_canny):
    ...
    Returns:
        [type]: [description]
    """
    lines = cv2.HoughLines(thresh_canny,1, np.pi / 180, 200,min_theta=math.pi/8,max_theta=7*math.pi/8)
    return lines
